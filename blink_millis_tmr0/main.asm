...
;	.line	63; main.c	current = tmr0_millis();
	...
	BANKSEL	_current
	MOVWF	_current, B
	MOVFF	PRODL, (_current + 1)
	MOVFF	PRODH, (_current + 2)
	MOVFF	FSR0L, (_current + 3)
	BANKSEL	_previous
;	.line	64; main.c	if((current - previous) >= 1000){
	...
	MOVWF	r0x00
	BANKSEL	(_previous + 1)
	MOVF	(_previous + 1), W, B
	BANKSEL	(_current + 1)
	SUBWFB	(_current + 1), W, B
	MOVWF	r0x01
	BANKSEL	(_previous + 2)
	MOVF	(_previous + 2), W, B
	BANKSEL	(_current + 2)
	SUBWFB	(_current + 2), W, B
	MOVWF	r0x02
	BANKSEL	(_previous + 3)
	MOVF	(_previous + 3), W, B
	BANKSEL	(_current + 3)
	SUBWFB	(_current + 3), W, B
	MOVWF	r0x03
	MOVLW	0x00
	SUBWF	r0x03, W
	BNZ	_00206_DS_
...
;	.line	65; main.c	previous = current;
	MOVFF	_current, _previous
	MOVFF	(_current + 1), (_previous + 1)
	MOVFF	(_current + 2), (_previous + 2)
	MOVFF	(_current + 3), (_previous + 3)
...
;	.line	117; ../my_sdcc_lib/rosso_sdcc_isr_delay.h	TMR0L = tmr0_load;
	...
	BANKSEL	_tmr0_isr_walker
;	.line	120; ../my_sdcc_lib/rosso_sdcc_isr_delay.h	tmr0_isr_walker = tmr0_isr_walker + 1;
	INCF	_tmr0_isr_walker, F, B
	BNC	_00158_DS_
; removed redundant BANKSEL
	INCF	(_tmr0_isr_walker + 1), F, B
	BNC	_00158_DS_
; removed redundant BANKSEL
	INCFSZ	(_tmr0_isr_walker + 2), F, B
	BRA	_10193_DS_
; removed redundant BANKSEL
	INCF	(_tmr0_isr_walker + 3), F, B
_10193_DS_:
_00158_DS_:
...
;	.line	105; ../my_sdcc_lib/rosso_sdcc_isr_delay.h	tmr0_isr_walker = 0;
	CLRF	_tmr0_isr_walker, B
; removed redundant BANKSEL
	CLRF	(_tmr0_isr_walker + 1), B
; removed redundant BANKSEL
	CLRF	(_tmr0_isr_walker + 2), B
; removed redundant BANKSEL
	CLRF	(_tmr0_isr_walker + 3), B
	RETURN	
...
;	.line	34; ../my_sdcc_lib/rosso_sdcc_isr_delay.h	temp = tmr0_isr_walker;
	MOVFF	_tmr0_isr_walker, r0x00
	MOVFF	(_tmr0_isr_walker + 1), r0x01
	MOVFF	(_tmr0_isr_walker + 2), r0x02
	MOVFF	(_tmr0_isr_walker + 3), r0x03
